...
class ShowTime(models.Model):
    ...
    def _find_overlapping_shows(self, buffer_minutes=BUFFER_MINUTES):
        ...
        for s in qs.select_related("movie"):
            ...
            other_end = s.start_time + timedelta(
                minutes=s.movie.duration + buffer_minutes
            )
    ...
    def clean(self):
        ...
        if not self.movie and self.movie.duration is None:
            raise ValidationError(
                {
                    "movie": "Movie must have duration (minutes) set before creating showtime."
                }
            )
        ...
        if overlaps:
            conflicts = ", ".join(
                [
                    f"{o.movie.title} at {o.start_time.strftime('%Y-%m-%d %H:%M')}"
                    for o in overlaps
                ]
            )
            raise ValidationError(
                {
                    "start_time": f"Overlaps with existing show(s) in this hall: {conflicts}"
                }
            )
